...
int main(int argc, char* argv[])
{
    ...
    for (int i = 0; i < input_num; i ++) {
        ...
        input_shape = (size_t*)malloc(sizeof(size_t) * input_dim);
    ...
    for (int i = 0; i < output_num; i ++) {
        ...
        output_shape = (size_t*)malloc(sizeof(size_t) * output_dim);
        ORT_ABORT_ON_ERROR(g_ort->GetDimensions(tensor_info, output_shape, output_dim));
    }

    size_t input_tensor_size = 2 * 3;
    float * input_tensor_values = (float*)malloc(sizeof(float) * input_tensor_size);
    for (size_t i = 0; i < input_tensor_size; i ++)
        input_tensor_values[i] = i;

    OrtMemoryInfo* memory_info;
    ORT_ABORT_ON_ERROR(g_ort->CreateCpuMemoryInfo(OrtArenaAllocator, OrtMemTypeDefault, &memory_info));

    OrtValue* input_tensor = NULL;
    ORT_ABORT_ON_ERROR(g_ort->CreateTensorWithDataAsOrtValue(memory_info, (void*)input_tensor_values, input_tensor_size * sizeof(float), input_shape, input_dim, ONNX_TENSOR_ELEMENT_DATA_TYPE_FLOAT, &input_tensor));
    ...
    float* floatarr;
    ORT_ABORT_ON_ERROR(g_ort->GetTensorMutableData(output_tensor, (void**)&floatarr));
